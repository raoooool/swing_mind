...
class PoseDetector:
    ...
    def detect(self, frame: np.ndarray) -> dict[str, Any]:
        ...
        Returns:
            {
                'detected': bool,
                'keypoints': List[Dict],  # [{'id', 'name', 'x', 'y', ...}]
    ...
    def _calculate_angles(self, keypoints: list[dict]) -> dict[str, float]:
        ...
        def get_point(name: str) -> np.ndarray | None:
            for kp in keypoints:
                if kp["name"] == name:
        ...
        if r_shoulder is not None and l_shoulder is not None:
            ...
            angles["shoulder_rotation"] = self._angle_between_vectors(
                shoulder_vec, horizontal
            )
    ...
    def draw_pose(self, frame: np.ndarray, pose_result: dict[str, Any]) -> np.ndarray:
        ...
        for kp in pose_result["keypoints"]:
            if kp["visibility"] > 0.5:
                cv2.circle(annotated, (int(kp["x"]), int(kp["y"])), 5, (0, 255, 0), -1)
